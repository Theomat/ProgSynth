...
from calculator.calculator import FLOAT
...
def basic_output_validator(
    float_lexicon: TList[float], max_list_length: int
) -> Callable[[Any], bool]:
    def validate_output(output: Any) -> bool:
        if isinstance(output, bool):
            return True
        elif isinstance(output, int):
            return float(output) in float_lexicon
        elif isinstance(output, float):
            return output in float_lexicon
...
def reproduce_dataset(
    ...
    float_range: TList[float] = [99999999.9, 0]
    float_range[1] = -int_range[0]
